...
declare -r xctoolVars=$(xctool -project PMDate.xcodeproj -scheme PMDateTests -sdk iphonesimulator -configuration Debug -showBuildSettings | egrep '(BUILT_PRODUCTS_DIR)|(CURRENT_ARCH)|(OBJECT_FILE_DIR_normal)|(SRCROOT)|(OBJROOT)' |  egrep -v 'Pods')
while read line; do
